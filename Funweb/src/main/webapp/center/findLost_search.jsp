...
// 				}
				...
				for(FileBoardDTO board : boardList) {
					%>
					<tr onclick="location.href='findLost_content.jsp?num=<%=board.getNum()%>'">
						<td><%=board.getNum() %></td>
...
<%-- 						<td><%=board.getDate() %></td> --%>
			...
			%>
				...
				<%} else { %>
					<a href="findLost_search.jsp?keyword=<%=keyword %>&pageNum=<%=pageNum - 1%>">Prev</a>
				<%} %>
				...
				<%for(int i = startPage; i <= endPage; i++){%>
					...
					<%} else { %>
						<a href="findLost_search.jsp?keyword=<%=keyword %>&pageNum=<%=i%>"><%=i%></a>
					<%} %>
				<%} %>
				...
				<%} else { %>
					<a href="findLost_search.jsp?keyword=<%=keyword %>&pageNum=<%=pageNum + 1%>">Next</a>
				<%} %>
				<div id="table_search">
				<form action="findLost_search.jsp" method="get">
					...
					<input type="button" value="글쓰기" style="cursor:pointer" class="btn" onclick="location.href='findLost_write.jsp'">
				</form>
				</div>
			</div>
		</article>
